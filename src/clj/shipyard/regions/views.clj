...
(def attrs (merge workspace/transition-attrs {:hx-post "/parts/regions" :hx-target "#part-regions" :hx-swap "outerHTML"
                                              :hx-include "#region-stroke input[name=mode]"
         ...
         (when detail?
           [:div.region-layer__actions
            ...
            [:form.region-layer__delete
             (assoc attrs :hx-confirm (str "Delete layer “" layer "” from every part in this library? Its painted regions will return to Primary."))
             (fields part-id mesh-key regions)
             [:input {:type "hidden" :name "layer" :value layer}]
             [:input {:type "hidden" :name "action" :value "delete"}]
             [:input {:type "hidden" :name "confirmed" :value "true"}]
             [:button {:type "submit" :aria-label (str "Delete " layer) :title "Delete from every part"}
              [:span {:aria-hidden "true"} "×"]]]])]
        (when (and detail? local? (not stale?))
...
(defn panel
  ...
  ([part-id mesh-key saved selected error shared-layers]
   (panel part-id mesh-key saved selected error shared-layers "facets"))
  ([part-id mesh-key saved selected error shared-layers mode]
   (let [regions (or saved (model/empty-regions mesh-key))
     ...
     [:section#part-regions {:data-regions (pr-str preview) :data-mesh-key mesh-key :data-part-id part-id}
      ...
      (layer-list part-id mesh-key regions available selected stale?)
      (if stale?
        ...
        (list
         ...
         [:form#region-stroke (assoc attrs :hx-sync "this:drop")
          ...
          [:p.muted "Facets paints individual triangles; Faces extends across connected flat surfaces."]
         ...
         [:form#region-fill (assoc attrs :hx-include "#region-stroke input[name=layer], #region-stroke input[name=mode]")
          (fields part-id mesh-key regions)
